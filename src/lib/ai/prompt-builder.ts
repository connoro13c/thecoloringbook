...
const STYLE_PROMPTS = {
  classic: {
    ...
    characteristics:
      "clean bold outlines, simple shapes, clear distinct sections",
  ...
  ghibli: {
    ...
    characteristics:
      "beautiful detailed illustrations, organic flowing lines, magical elements",
  ...
  mandala: {
    ...
    characteristics:
      "intricate geometric patterns, symmetrical designs, decorative elements",
...
export function buildColoringPagePrompt({
  ...
  const difficultyModifier =
    DIFFICULTY_MODIFIERS[difficulty as keyof typeof DIFFICULTY_MODIFIERS];
  ...
  const recommendedElements =
    photoAnalysis.suggestions.recommendedElements.join(", ");
...
export function buildDallePrompt(components: PromptComponents): string {
  ...
  const prompt = `Create a black-and-white ${
    styleConfig.artStyle
  } coloring page illustration featuring this magical scene: ${sceneDescription}
...
- The main character must be recognizable as: ${photoAnalysis.child.age} ${
    photoAnalysis.child.gender
  }
- ESSENTIAL physical characteristics to preserve: ${
    photoAnalysis.child.appearance
  }
- Key clothing/accessories to maintain: ${photoAnalysis.child.clothing}
- Facial expression should adapt to the scene while showing: ${
    photoAnalysis.child.expression
  }
...
Complexity: Level ${difficulty}/5 - ${
    photoAnalysis.suggestions.coloringComplexity
  }`;
